...
class BinanceService:
    ...
    async def notify_price_go_up(
        ...
        if user_no in settings.binance_data['Aleshka_No']:
            ...
            message += f'\n\n💸 Твой спред: {round((settings.number-max_price/58.9)*100*100)/100}% 💸🪙'
        else:
    ...
    async def get_max_price_data():
        ...
        if len(json_res['data']) > 1:
            is_duplicate_price = \
                float(json_res['data'][1]['adv']['price']) == float(json_res['data'][3]['adv']['price'])
            duplicate_nickname = json_res['data'][3]['advertiser']['nickName']

        return (
            float(json_res['data'][1]['adv']['price']),
            json_res['data'][1]['advertiser']['userNo'],
            json_res['data'][1]['advertiser']['nickName'],
            duplicate_nickname,
            is_duplicate_price
        )
